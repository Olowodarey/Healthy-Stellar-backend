...
export class QualityControlService {
    ...
    async create(
        createDto: CreateQualityControlDto,
        userId: string,
        ...
        const qcLog = this.qcLogRepository.create({
            ...
            nextDueDate: createDto.nextDueDate
                ? new Date(createDto.nextDueDate)
                : undefined,
            createdBy: userId,
            updatedBy: userId,
        });
    ...
    async getQCStatistics(equipmentId: string, days: number = 30): Promise<{
        totalTests: number;
        passedTests: number;
        failedTests: number;
        passRate: number;
        ...
        const lastCalibration = logs.find(
            (log) => log.qcType === QCType.CALIBRATION,
        )?.qcDate;
