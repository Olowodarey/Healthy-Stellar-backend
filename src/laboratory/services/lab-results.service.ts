...
export class LabResultsService {
    ...
    async create(
        ...
        if (!orderItem) {
            throw new NotFoundException(
                `Order item with ID ${createDto.orderItemId} not found`,
            );
        }
        ...
        const labResult = this.labResultRepository.create({
            ...createDto,
            performedDate: createDto.performedDate
                ? new Date(createDto.performedDate)
                : new Date(),
        ...
        for (const valueDto of createDto.values) {
            ...
            if (!parameter) {
                throw new NotFoundException(
                    `Parameter with ID ${valueDto.parameterId} not found`,
                );
            }
            ...
            if (valueDto.numericValue !== undefined && valueDto.numericValue !== null) {
                resultValue.abnormalFlag = this.determineAbnormalFlag(
                    valueDto.numericValue,
                    parameter,
                );
    ...
    async correct(
        ...
        for (const valueDto of newValues) {
            const existingValue = labResult.values.find(
                (v) => v.parameterId === valueDto.parameterId,
            );
    ...
    private async createCriticalAlerts(
        labResult: LabResult,
        providerId: string,
    ): Promise<void> {
