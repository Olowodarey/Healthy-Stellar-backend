...
export class SpecimensService {
    ...
    async create(
        ...
        if (existing) {
            throw new ConflictException(
                `Specimen with ID ${createDto.specimenId} already exists`,
            );
        }
        ...
        if (!labOrder) {
            throw new NotFoundException(
                `Lab order with ID ${createDto.labOrderId} not found`,
            );
        }
    ...
    async findBySpecimenId(specimenId: string): Promise<Specimen> {
        ...
        if (!specimen) {
            throw new NotFoundException(
                `Specimen with specimen ID ${specimenId} not found`,
            );
        }
